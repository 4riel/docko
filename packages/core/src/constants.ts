...
// The managed writable tree. Every consumer must use this name instead of spelling it out, so a
// path check and slot discovery can never disagree about what "inside slots/" means.
...
// A stale lock is broken by renaming it aside first, so only the renaming winner may delete it.
// The quarantined directory carries this prefix so the temp sweeper reclaims it after a crash.
export const STALE_LOCK_DIR_PREFIX = `${MUTATION_LOCK_DIR}.stale-`;
...
// Upper bound for refreshing an owner claim heartbeat on the authorized-write path.
export const CLAIM_WRITE_HEARTBEAT_THROTTLE_MS = 30 * 1000;
// Lower bound for the same throttle, so a very short stale window still costs at most one
// registry write per second on an edit-heavy session.
export const CLAIM_WRITE_HEARTBEAT_MIN_THROTTLE_MS = 1000;
